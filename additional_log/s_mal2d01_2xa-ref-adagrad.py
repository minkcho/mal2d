...
filename = '/home/minkcho/proj/malware/vir_sample_' + str(byte_size) + '_kisa_2a.npz'
...
def create_model(x = 16, y = 64):   
    ...
    inp = Input(batch_shape=(None, input_data_size, input_data_size*8, 1), name='Input')
    ...
    basemodel.compile( loss='binary_crossentropy', optimizer=Adagrad(0.001), metrics=['accuracy']) # , weight_pred] )
 
    return basemodel, "mal2d01"
...
def evalute_model(model, threshold=0.9):   
    filename = '/home/minkcho/proj/malware/train_' + str(byte_size) + '_ms_2a.npz'
...
def test_model(model, threshold=0.9):
    filename = '/home/minkcho/proj/malware/renameAll_' + str(byte_size) + '_msorg_2a.npz'
   ...
   # result2 = model.predict(x=ttest['x_valid'])
   # print(result2)
   # result_sum += np.sum(result2 < threshold)
...
#    result4 = model.predict(x=test['x_valid'])
    y_test3 = test['y_train']
#    y_test4 = test['y_valid']
