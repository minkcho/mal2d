...
filename = base_dir + 'vir_sample_' + str(byte_size) + '_kisa_2a.npz'
...
def evalute_model(model, threshold=0.9):   
    filename = base_dir + 'train_' + str(byte_size) + '_ms_2a.npz'
...
def test_model(model, threshold=0.9):
    filename = base_dir + 'renameAll_' + str(byte_size) + '_msorg_2a.npz'
    ttest = np.load(filename)

    my_size = len(ttest['x_train']) 
    print(my_size)
